...
compression as a function of the distribution of the elements of the 
matrix, some results comparing our method with the traditional method of matrix 
allocation, and arithmetic operations as a benchmark measure the efficiency of 
operating with bitstring-compressed matrices.
...
inefficiencies in the conventional way matrices are allocated in memory. 
 The analyses presented in this article concern arrays of positive integers, 
but can be applied to signed integers and real numbers with few adaptations. 
...
(power of 2 number of bits), one for each element. The maximum size of a block 
...
will be the most efficient for most matrices. With the VLB 
method the resulting matrix is smaller than the original matrix.
...
one exception: for bit-length 64, where neither method is able to compress 
the data. 
...
or have nearly-constant bit-length, the SM method is better, otherwise, the VLB 
...
the groups, $\frac{f_i}{rc}$, while sticking to two groups. Here, we assume 
that $\frac{f_i}{rc}$ is a good approximation to the 
...
\paragraph{Numerical Estimates:}
The results for this simulation can be seen in table \ref{tab:06}. Note that 
...
\section*{Implementation }\todo{acho que esta seção toda deve ir para o 
...
open-source license\todo{colocar o link para o repositório aqui}. The 
...
is applied to thein accordance to traditional methods\todo{acho que esta 
explicação ficou muito solta. Não precisamos explica a implementação no 
artigo, podemos colocar na documentação do código}.
...
\section*{Library Application}\todo{Também deve ir para o material suplementar}
...
BFM Methods.  A computer with 8 GB of RAM Memory, 16 GB of swap, processor core i7 model 870  \todo{confirmar modelo}
and operational 
...
manuscript.\todo{Adicionar a Carol}
