...
def main():
    ...
    P.axis([0,tam,0,tam])
    P.colorbar(spacing='uniform', ticks=range(1, 10))
    P.title('Addition')
    fig.add_subplot(222)
    P.pcolor(submat, cmap=lcm)
    P.axis([0,tam,0,tam])
    P.colorbar(spacing='uniform', ticks=range( 9))
    P.title('Subtraction')
    fig.add_subplot(223)
    P.pcolor(mulmat, cmap=lcm)
    P.axis([0,tam,0,tam])
    P.colorbar(spacing='uniform', ticks=range(1, 17))
    ...
    P.title('Division')
    P.colorbar(spacing='uniform', ticks=range( 9))
    P.savefig('bitlength.tif',dpi=300)
    P.show()
